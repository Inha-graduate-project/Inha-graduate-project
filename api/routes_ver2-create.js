...
const route_ver2 = require('../route/route_ver2');

async function saveRoute_ver2(req, res) { // 비동기적 동작
    const userId = req.params.user_id; // 요청에서 user_id 파라미터를 가져오기

    try {
        const user_info_list = await route_ver2(userId); // 경로 정보 배열
...
module.exports = saveRoute_ver2; // saveRoute 함수를 모듈로 export